  ...
  const renderSubMenu = (
    ...
    switch (expandedMenu) {
      case menus[0]:
        ...
        break;
      case menus[1]:
        subMenuItem = morphMenus.map((menu) => (
          <ul className="p-1 bg-slate-800 rounded-xs flex flex-col gap-1">
            <li
              onClick={() => handleNavigate("morphologi", menu.path)}
              className="m-1 p-1 hover:bg-slate-600">
              {menu.name}
            </li>
          </ul>
        ));
        break;
      case menus[2]:
        subMenuItem = phraseMenus.map((menu) => (
          <ul className="p-1 bg-slate-800 rounded-xs flex flex-col gap-1">
            <li
              onClick={() => handleNavigate("phrase", menu.path)}
              className="m-1 p-1 hover:bg-slate-600">
              {menu.name}
            </li>
          </ul>
      ...
      <ul className="p-2 flex flex-col gap-2">
        {menus.map((menu) => (
          <li
            ...
            className="p-1 bg-slate-600 rounded-xs flex flex-col items-stretch cursor-pointer">
